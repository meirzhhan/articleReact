...
describe('loginByUsername.test', () => {
  ...
  test('success login', async () => {
    ...
    thunk.api.post.mockReturnValue(Promise.resolve({ data: userValue })); // Сценарий возвращения userValue
    ...
    expect(thunk.api.post).toHaveBeenCalled(); // проверка вызова api.post(thunk)
    expect(result.meta.requestStatus).toBe('fulfilled'); // проверка статуса
    expect(result.payload).toEqual(userValue); // return response.data
  });
  ...
  test('error login', async () => {
    ...
    thunk.api.post.mockReturnValue(Promise.resolve({ status: 403 })); // Сценарий возращения статус кода 403
    const result = await thunk.callThunk({ username: '123', password: '123' });
    // console.log(result);
